...
const string strMessageMagic = "SevenCoin Signed Message:\n";
  ...
  if(pindexBest->nHeight+1 >= 1 && pindexBest->nHeight+1 < 2)
{
  ...
  else if(pindexBest->nHeight+1 >= 2 && pindexBest->nHeight+1 <= 193190)
{
  nSubsidy = 1 * COIN;
}
  else if(pindexBest->nHeight+1 >= 193191 && pindexBest->nHeight+1 <= 593190)
{
  nSubsidy = 0.5 * COIN;
}
  else if(pindexBest->nHeight+1 >= 593191 && pindexBest->nHeight+1 <= 1393190)
{
  nSubsidy = 0.25 * COIN;
}
  else if(pindexBest->nHeight+1 >= 1393191 && pindexBest->nHeight+1 <= 3393190)
{
  nSubsidy = 0.1 * COIN;
}
  else if(pindexBest->nHeight+1 >= 3393191 && pindexBest->nHeight+1 <= 11393190)
{
  nSubsidy = 0.025 * COIN;
}
  else if(pindexBest->nHeight+1 >= 11393191 && pindexBest->nHeight+1 <= 31393190)
{
  nSubsidy = 0.01 * COIN;
}
...
// SevenCoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SevenCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "According to Hinduism, there are seven worlds in the universe, seven seas and seven Rishies called sapta rishis.";
        CTransaction txNew;
        txNew.nTime = 1491446762;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x3768306b8a11f0dba43f73b4668b81d6f0b7b562b157f12760f94870d8406661"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xb5, 0xb6, 0xb7, 0xd1 };
